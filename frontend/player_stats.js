...
if (!playerId) {
    document.getElementById('stats-body').innerHTML = '<tr><td colspan="3">No player specified.</td></tr>';
} else {
    apiFetch(`/players/${playerId}`)
        .then(player => {
            document.getElementById('page-title').textContent = `${player.first_name} ${player.last_name} — Stats`;
        })
        .catch(() => {});
...
function loadStats() {
    apiFetch(`/players/${playerId}/stats`).then(stats => {
...
function saveStat(stat_id, btn) {
    ...
    apiFetch(`/players/${playerId}/stats/${stat_id}`, {
        method: 'PUT',
...
function addStat() {
    ...
    apiFetch(`/players/${playerId}/stats`, {
        method: 'POST',
        body: JSON.stringify(data)
    }).then(() => {
...
function deleteStat(stat_id) {
    apiFetch(`/players/${playerId}/stats/${stat_id}`, { method: 'DELETE' })
        .then(() => loadStats());
}
